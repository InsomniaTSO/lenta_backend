...
def aggregation(pd_cat, pd_sales, pd_forecast):
    """Расчет WAPE для предсказаний."""
    pd_sales["fact"] = pd_sales["sales_units"]
    pd_sales = pd_sales.drop(
        columns=[
            "sales_run_promo",
            "sales_rub",
            "sales_units_promo",
    ...
    pd_sal_for = (
        ...
        .fillna(0)
        .drop(columns=["id_y", "id_x"], axis=1)
    )
    pd_sal_for.fact = pd_sal_for.fact.astype(int)
    pd_sal_for = (
        pd_sal_for.groupby(["store_id", "product_id"])
        .agg({"target": ["sum"], "fact": ["sum"]})
        .reset_index()
    )
...
def aggregation_by_store(pd_all):
    """Расчет среднего WAPE для предсказаний по магазинам."""
    pd_all_gr = (
        pd_all.groupby(["date_range", "store_id"])
        .agg({"target": ["sum"], "fact": ["sum"],
              "delta": ["mean"], "WAPE": ["mean"]})
        .reset_index()
    )
