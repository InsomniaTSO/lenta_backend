...
class ForecastPostSerializer(serializers.ModelSerializer):
    ...
    Позволяет отправить информацию о магазине,
    дате прогноза и прогнозе продаж.
    """
    ...
    def create(self, data):
        ...
        for key, value in forecast.items():
            obj = Forecast.objects.filter(
                store=store, product=sku, forecast_date=forecast_date, date=key
            )
            if obj.exists():
...
class ForecastGetSerializer(serializers.ModelSerializer):
    ...
    Предоставляет информацию о магазине, продукте
    и прогнозе продаж.
    """
    ...
    def get_forecast(self, obj):
        ...
        forecasts = Forecast.objects.filter(
            store=store, product=product, forecast_date=forecast_date
        )
