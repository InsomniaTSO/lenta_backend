...
def aggregation(pd_cat, pd_sales, pd_forecast):
    ...
    pd_sales = pd_sales.drop(columns=['sales_run_promo', 'sales_rub',
                                      'sales_units_promo',
    ...
    pd_sal_for = pd.merge(pd_forecast, pd_sales, how='left',
                          on=['store_id', 'product_id', 'date'],
                          left_index=False,
                          right_index=False).fillna(0).drop(columns=['id_y',
                                                                     'id_x'],
                                                            axis=1)
    pd_sal_for.fact = pd_sal_for.fact.astype(int)
    pd_sal_for = pd_sal_for.groupby(['store_id',
                                     'product_id']).agg({'target': ['sum'],
                                                         'fact': ['sum']}) \
                                                   .reset_index()
    pd_sal_for.columns = ['store_id', 'product_id', 'target', 'fact']
    pd_all = pd.merge(pd_sal_for, pd_cat, how='left', on=['product_id'],
                      left_index=False, right_index=False).drop('uom',
                                                                axis=1)
...
def aggregation_by_store(pd_all):
    pd_all_gr = pd_all.groupby(['date_range',
                                'store_id']).agg({'target': ['sum'],
                                                  'fact': ['sum'],
                                                  'delta': ['mean'],
                                                  'WAPE': ['mean']}) \
                                            .reset_index()
    pd_all_gr.columns = ['date_range', 'store_id', 'target',
                         'fact', 'delta', 'WAPE']
