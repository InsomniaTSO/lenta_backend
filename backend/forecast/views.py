...
from forecast.filters import ForecastFilter
from forecast.get_xls import get_xls, get_quality_xls
from forecast.models import Forecast
from forecast.serializers import ForecastGetSerializer, ForecastPostSerializer
from sales.models import Sales
from categories.models import Product
from forecast.aggregation import aggregation, aggregation_by_store
...
class ForecastViewSet(viewsets.ModelViewSet):
    ...
    def get_serializer_class(self):
        """Возвращает сериализатор в зависимости от используемого метода."""
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            request.data["data"], status=status.HTTP_201_CREATED,
            headers=headers
        )
    ...
    def download_file(self, request):
        ...
        response = HttpResponse(forecast,
    ...
    def forecast_quality(self, request):
        ...
        if (not query_sales.filter(query_s) or not
                query_forecast.filter(query_f)):
            return Response({"data": []}, status=status.HTTP_404_NOT_FOUND)
        ...
        if group == "1":
            ...
            return Response(
                {"data": json.loads(pd_all.to_json(orient="records"))},
                status=status.HTTP_200_OK,
            )
